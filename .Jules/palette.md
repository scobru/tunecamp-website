...
## 2024-07-25 - Zero-Result Empty States
**Learning:** Found that empty states for filtering and searching only showed a static text message ("No tracks match your search"). Without a clear recovery path, users might feel stuck or manually clear inputs one by one.
**Action:** When creating zero-result empty states for filtering or searching, always include an actionable button (e.g., "Clear Filters") that programmatically resets input values and re-triggers the list render to provide a clear and easy recovery path.

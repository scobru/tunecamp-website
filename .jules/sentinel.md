...
## 2024-05-31 - XSS via unescaped innerHTML in press.html
**Vulnerability:** Object properties were directly inserted into an HTML string and assigned to `innerHTML` without escaping, which could lead to XSS if the data was dynamic.
**Learning:** Even statically defined data rendered via `innerHTML` without escaping represents an insecure pattern that violates secure coding principles and leaves the door open for future XSS if the data source becomes dynamic.
**Prevention:** Always use `escapeHtml` (or equivalent) when interpolating variables into template strings used for `innerHTML`, or prefer safe DOM manipulation methods (like `textContent`).

...
## 2024-05-20 - Debouncing Search Inputs to Prevent Main Thread Blocking
**Learning:** In the Tunecamp static frontend, rapid DOM updates via `innerHTML` replacement (such as regenerating the entire track grid during search filtering) cause significant main thread blocking. High-frequency event listeners like text inputs trigger these full DOM rebuilds on every keystroke.
**Action:** Always debounce high-frequency event listeners (like text inputs) that trigger full DOM rebuilds to prevent main thread blocking and improve responsiveness.

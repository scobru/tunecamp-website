...
## 2024-07-26 - Debouncing Search Inputs to Prevent Main Thread Blocking
**Learning:** In the Tunecamp static frontend architecture, rapid DOM updates via `innerHTML` replacement (such as regenerating the entire track grid during search filtering) cause significant main thread blocking. Firing these full DOM rebuilds synchronously on every keystroke (`oninput`) makes the application feel sluggish and unresponsive.
**Action:** Always debounce high-frequency event listeners (like text inputs) that trigger full DOM rebuilds to ensure the main thread remains free for user interactions.

...
## 2024-07-19 - Debounce Search Inputs for DOM Rebuilds
**Learning:** In the Tunecamp static frontend, rapid DOM updates via `innerHTML` replacement (such as regenerating the entire track grid during search filtering) cause significant main thread blocking. High-frequency event listeners (like text inputs) connected to these updates will severely degrade performance.

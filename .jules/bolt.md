...
## 2024-05-15 - Main Thread Blocking from High-Frequency DOM Updates
**Learning:** In the Tunecamp static frontend, rapid DOM updates via `innerHTML` replacement (such as regenerating the entire track grid during search filtering) cause significant main thread blocking if fired on every keystroke.
**Action:** Always debounce high-frequency event listeners (like text inputs) that trigger full DOM rebuilds to maintain UI responsiveness.
